...
final class SqlAddressStorage implements AddressStorageInterface
{
    ...
    public function listByCustomer(string $customerId): array
    {
        ...
        return array_map($this->hydrate(...), $this->db->rows('address_list_by_customer', ['customerId' => (int) $customerId]));
    }
    ...
    public function getById(string $addressId): AddressEntity|null
    {
        ...
        $row = $this->db->row('address_get', ['id' => (int) $addressId]);
    ...
    public function put(AddressEntity $address): void
    {
        ...
        $this->db->exec($this->db->row('address_exists', ['id' => $id]) === null ? 'address_insert' : 'address_update', $values);
    }
    ...
    {
        if (ctype_digit($addressId)) {
            $this->db->exec('address_delete', ['id' => (int) $addressId]);
        }
    }
