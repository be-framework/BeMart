 ...
 * (`authKey`) bound to the pending password-verified login challenge and
 * asks the admin to confirm by entering the first `deviceToken`.
 * EC-CUBE's `admin_two_factor_auth_set` controller stores the secret on
 * the member once the first code verifies. ALPS marks it `idempotent` —
 * registering the same secret twice is a no-op.
 */
