...
| **Phase B — security / production hardening** | Psalm taint setup, `ProdModule`, env-gated CLI entry point, EC-CUBE static-asset deployment (`default` + `admin` themes), and the HTTP router — Aura.Router via `RouteTable` maps EC-CUBE route name ↔ URL path ↔ resource URI; `public/index.php` dispatches with 404/405 semantics; `BeMartTwigExtension::url()/path()` resolve through the shared `RouteTable`. | `a002097` (asset deploy); `53e587e`/`39f1117` (HTTP router); `16e8c9d` (asset-package-aware render stubs) |
