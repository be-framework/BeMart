...
| 2. Be domain (`be/src`) | 139/144 transitions | Phase A migrated 139 transitions. 5 ALPS-only transitions have **no domain impl**; 7 Phase-A transitions are functional **stubs**. 124 Final / 123 Input / 134 Semantic / 14 Being / 38 Entity. |
...
4. **5 ALPS-only transitions — no domain impl.** Added by the Phase-3 ALPS remediation, never implemented in `be/src`: `doSortNoMove`, `doToggleVisible`, `doUpdateTrackingNumber`, `doSendShippingNotifyMail`, `doResendActivationMail`. (This is why domain coverage is **139 of 144**.)
