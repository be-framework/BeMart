...
| 3. BEAR Resource / Router | 259 RouteTable entries / 293 method entries | Every public HTML route method has an ALPS descriptor (`293/293`). Concrete Resource dispatch is available for 235 method entries; 58 method entries intentionally use `ActionRedirect` as a safe fallback/return transition and are marked partial in the feature table. |
...
- **Route/ALPS gate: 293/293** — every RouteTable method entry resolves to an ALPS descriptor. See the generated HTML table: [`docs/eccube-feature-alps-status.html`](eccube-feature-alps-status.html).
...
| **Phase B — security / production hardening** | Psalm taint setup, `ProdModule`, env-gated CLI entry point, EC-CUBE static-asset deployment (`default` + `admin` themes), and the HTTP router — `RouteTable` + `Router` map EC-CUBE route name ↔ URL path ↔ resource URI; `public/index.php` dispatches with 404/405 semantics; `BeMartTwigExtension::url()/path()` resolve through the shared `RouteTable`. | `a002097` (asset deploy); `53e587e`/`39f1117` (HTTP router); `16e8c9d` (asset-package-aware render stubs) |
...
1. **ALPS route gate — done.** ✓ Done. `RouteTable` has 259 entries / 293 method entries; all 293 have an ALPS descriptor and every descriptor exists in `alps.json`. `unsupported-route` is 0 and template `PUT`/`DELETE` usage is 0.
2. **ActionRedirect functionalization.** 58 method entries still use `page://self/action-redirect`. They are no longer broken links: each has an ALPS ID and safe return semantics. They are still marked **partial** until the corresponding business operation is implemented as a concrete Resource/Final path. The generated status table is the working list.
