...
| doRegisterProductClass | [<code>page://self/admin/product/product-class</code>](/admin/product/product-class.md) |
## POST
...
| Name | Type | Description | Default | Required | Constraints | Example |
|------|------|-------------|---------|----------|-------------|---------|
| productCode | string | 商品コード - 登録対象の親商品を識別するSKU/品番。フォームの hidden camelCase フィールドから供給される。在庫・カート・受注明細の結合キー。 |  | Required | {"minLength":1,"maxLength":64} | sample-001 |
| price02 |  | 販売価格 - 登録する規格(SKU)の販売価格（税抜）。日本円の整数金額。ブラウザは text input を文字列として送信し、空欄は空文字列で届くため、transport では string/null も受け入れて Resource 境界で int に正規化する。 | 0 | Optional | {"$comment":"Request schema is transport-level; a browser posts text inputs as strings and blank fields as empty strings. Business invalid values are allowed through to Resource/Semantic validation."} | 1200 |
| stock |  | 在庫数 - 登録する規格(SKU)の物理在庫数。stockUnlimited=true の場合は無視される。ブラウザは文字列として送信し、空欄は空文字列で届く。 | 0 | Optional | {"default":0} | 10 |
| stock_unlimited |  | 在庫無制限（規格セル） - EC-CUBE 商品規格マトリクスの checkbox。チェック時のみ送信される。単一行ポートでは camelCase stockUnlimited を権威とする。 |  | Optional |  | 1 |
| delivery_fee |  | 商品別送料（規格セル） - EC-CUBE 商品規格マトリクスの送料入力。単一行ポートでは camelCase deliveryFee を権威とする。 | 0 | Optional | {"minLength":0,"maxLength":12} | 0 |
| product_code | string | 商品コード（規格セル） - EC-CUBE 商品規格マトリクスの 新規規格 セルが送信する SKU コード入力。単一行ポートでは登録には用いず、親 productCode を権威とする。 |  | Optional | {"minLength":0,"maxLength":64} | sample-001 |


### Response

[Object: POST /admin/product/product-class response](../schemas/post-admin-product-product-class.json)

| Name | Type | Description | Required | Constraints | Example |
|------|------|-------------|----------|-------------|---------|
| productClassId | string | 商品規格ID - 登録された dtb_product_class.id の不透明な文字列ハンドル。BeMart の RegisteredProductClassEntity 層は数値ではなく文字列として保持する。Fake 実装は採番フィクスチャ値を返し、SQL 実装は dtb_product_class.id (int unsigned AUTO_INCREMENT) を文字列化して使用（同インターフェイス・異 ID 形状）。classCategoryId / deliveryId と同じ Fake↔SQL 二重性。 | Required | {"minLength":1,"maxLength":128,"pattern":"^[A-Za-z0-9._:@/-]+$"} | pc-fake-next |
| productCode | string | 商品コード - 登録した規格(SKU)の親商品を識別するSKU/品番。在庫・カート・受注明細の結合キー。 | Required | {"minLength":0,"maxLength":64} | sample-001 |
