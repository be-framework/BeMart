...
| [`event-sourcing-observation.md`](event-sourcing-observation.md) | **event-sourcing 観測ログの credential 契約の正**。`AppParamsFilter`/`ExcludedResponseBodyStore`/`#[SensitiveParameter]` の redaction 範囲、既知のギャップ、`replayable:false` が成功リクエストもイベントストリームから除外する運用上の帰結。 |
