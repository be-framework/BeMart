...
| ディレクトリ | 内容 |
|---|---|
| [`methodology/`](methodology/) | 再利用可能な方法論・原則。`standard-first-rule.md`（BEAR境界を触る前の標準参照ルール）、`architecture-audit-baseline.md`（BEAR境界標準逸脱の監査baseline）、`ai-standardization-retrospective.md`（AI独自実装の反省）、`hypermedia-test-principle.md`（hypermedia テスト＝移植契約）、`FRAMEWORK_REVIEW.md`（元139 transition sliceの歴史的フレームワーク実務レビュー）、`be-bear-ai-era.md`（Be + BEAR の AI コーディング時代における潜在能力の論評）。 |
