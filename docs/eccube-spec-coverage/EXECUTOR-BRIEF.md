...
```
seed 管理者: `test-admin` / `admin-test-password-2026`
...
```bash
JAR=/tmp/bemart.cookies
# 1) POST /admin/login (loginId=test-admin, password=admin-test-password-2026, csrfToken=§3aで抽出) → /admin/two-factor-auth へ redirect
