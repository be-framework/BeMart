...
標準 workflow は [`packet-lifecycle.json`](~/git/ec-cube-alps/.migrate/workflows/packet-lifecycle.json) です。`semantic -> generate -> implement -> review` を通し、`review` が exit code `10` を返した場合だけ `fix -> review` へ遷移します。

`workflow` は resource ごとに分けず、すべて [`php bin/orchestrator packet run <step>`](~/git/ec-cube-alps/bin/orchestrator) の generic executor を呼びます。
