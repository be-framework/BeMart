...
2. `git log --oneline -1` shows a new commit on branch cache-app-layer since the previous iteration.
3. The commit added at least one `app://self/…` resource and changed a Page resource to use `#[Embed]`, and no Page resource it touched still injects a `*QueryInterface` (`git show --stat HEAD`).
4. The number of KNOWN entries in var/loop/verify-cache.php did not grow: compare
   `git show HEAD~1:var/loop/verify-cache.php | grep -c "=> 'bearsunday"` with
   `grep -c "=> 'bearsunday" var/loop/verify-cache.php`. Entries there are tracked
