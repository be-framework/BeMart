...
SELECT CAST(JSON_VALUE(CAST(:favorite AS CHAR), '$.customerId') AS UNSIGNED), p.id, NOW(), NOW(), 'customerfavoriteproduct'
FROM dtb_product p
INNER JOIN dtb_product_class pc ON pc.product_id = p.id
WHERE JSON_VALUE(CAST(:favorite AS CHAR), '$.customerId') REGEXP '^[0-9]+$'
  AND pc.product_code = JSON_VALUE(CAST(:favorite AS CHAR), '$.productCode')
