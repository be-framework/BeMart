...
SELECT CAST(JSON_VALUE(CAST(:news AS CHAR), '$.newsId') AS UNSIGNED), JSON_VALUE(CAST(:news AS CHAR), '$.newsTitle'), JSON_VALUE(CAST(:news AS CHAR), '$.newsDescription'), JSON_VALUE(CAST(:news AS CHAR), '$.newsUrl'), JSON_VALUE(CAST(:news AS CHAR), '$.publishDate'), IF(JSON_VALUE(CAST(:news AS CHAR), '$.linkMethod') = 'true', 1, 0), 1, NOW(), NOW(), 'news'
