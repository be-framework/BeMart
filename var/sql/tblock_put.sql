...
SELECT CAST(JSON_VALUE(CAST(:block AS CHAR), '$.blockId') AS UNSIGNED), JSON_VALUE(CAST(:block AS CHAR), '$.blockName'), JSON_VALUE(CAST(:block AS CHAR), '$.blockFileName'), IF(LOWER(JSON_VALUE(CAST(:block AS CHAR), '$.blockDeletable')) IN ('true', '1'), 1, 0), NOW(), NOW(), 'block'
