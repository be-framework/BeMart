...
-> Select #3 (subquery in projection; dependent)
    -> Aggregate: group_concat(json_quote(c.category_name) order by c.hierarchy ASC,c.sort_no DESC,c.id ASC separator ',')  (cost=0.8 rows=1) (never executed)
        -> Nested loop inner join  (cost=0.7 rows=1) (never executed)
            -> Covering index lookup on pcat using PRIMARY (product_id=p.id)  (cost=0.35 rows=1) (never executed)
            -> Single-row index lookup on c using PRIMARY (id=pcat.category_id)  (cost=0.35 rows=1) (never executed)
-> Select #4 (subquery in projection; dependent)
    -> Aggregate: group_concat(json_quote(t.`name`) order by t.sort_no ASC,t.id ASC separator ',')  (cost=0.8 rows=1) (never executed)
        -> Nested loop inner join  (cost=0.7 rows=1) (never executed)
            -> Filter: (pt.tag_id is not null)  (cost=0.35 rows=1) (never executed)
                -> Index lookup on pt using IDX_4433E7214584665A (product_id=p.id)  (cost=0.35 rows=1) (never executed)
            -> Single-row index lookup on t using PRIMARY (id=pt.tag_id)  (cost=0.35 rows=1) (never executed)
-> Select #5 (subquery in projection; dependent)
    -> Aggregate: group_concat(distinct json_quote(cn.`name`) order by cn.`name` ASC separator ',')  (cost=3303..3303 rows=1) (never executed)
        -> Index lookup on cn using <auto_key1> (product_id=p.id)  (cost=2542..2739 rows=564) (never executed)
            -> Union materialize with deduplication  (cost=2542..2542 rows=5636) (never executed)
                -> Nested loop inner join  (cost=375 rows=3636) (never executed)
                    -> Nested loop inner join  (cost=4 rows=20) (never executed)
                        -> Table scan on cn1  (cost=0.75 rows=5) (never executed)
                        -> Covering index lookup on cc1 using IDX_9B0D1DBAB462FB2A (class_name_id=cn1.id)  (cost=0.331 rows=4) (never executed)
                    -> Covering index lookup on pc1 using idx_bemart_pc_default_order (class_category_id1=cc1.id)  (cost=1.29 rows=182) (never executed)
                -> Nested loop inner join  (cost=1603 rows=2000) (never executed)
                    -> Nested loop inner join  (cost=903 rows=2000) (never executed)
                        -> Filter: (pc2.class_category_id2 is not null)  (cost=203 rows=2000) (never executed)
                            -> Index scan on pc2 using idx_bemart_pc_default_order  (cost=203 rows=2000) (never executed)
                        -> Filter: (cc2.class_name_id is not null)  (cost=0.25 rows=1) (never executed)
                            -> Single-row index lookup on cc2 using PRIMARY (id=pc2.class_category_id2)  (cost=0.25 rows=1) (never executed)
                    -> Single-row index lookup on cn2 using PRIMARY (id=cc2.class_name_id)  (cost=0.25 rows=1) (never executed)

### SHOW WARNINGS
[{"Level":"Note","Code":1276,"Message":"Field or reference 'eccubedb_test.p.id' of SELECT #2 was resolved in SELECT #1"},{"Level":"Note","Code":1276,"Message":"Field or reference 'eccubedb_test.p.id' of SELECT #3 was resolved in SELECT #1"},{"Level":"Note","Code":1276,"Message":"Field or reference 'eccubedb_test.p.id' of SELECT #4 was resolved in SELECT #1"},{"Level":"Note","Code":1276,"Message":"Field or reference 'eccubedb_test.p.id' of SELECT #5 was resolved in SELECT #1"}]
