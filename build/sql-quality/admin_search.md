...
+- Table scan
   +- Table
      ...
      rows            5
      filtered        100.00
      condition       (locate('管理者',`eccubedb_test`.`dtb_member`.`name`) > 0)
```
## Analysis Detail
...
### EXPLAIN JSON
{"select_id":1,"cost_info":{"query_cost":"5.75"},"ordering_operation":{"using_filesort":true,"cost_info":{"sort_cost":"5.00"},"table":{"table_name":"dtb_member","access_type":"ALL","rows_examined_per_scan":5,"rows_produced_per_join":5,"filtered":"100.00","cost_info":{"read_cost":"0.25","eval_cost":"0.50","prefix_cost":"0.75","data_read_per_join":"35K"},"used_columns":["id","work_id","authority_id","name","login_id","password","sort_no"],"attached_condition":"(locate('\u7ba1\u7406\u8005',`eccubedb_test`.`dtb_member`.`name`) > 0)"}}}

### EXPLAIN ANALYZE
-> Limit: 50 row(s)  (cost=0.75 rows=5) (actual time=0.0126..0.0135 rows=5 loops=1)
    -> Sort: dtb_member.login_id, limit input to 50 row(s) per chunk  (cost=0.75 rows=5) (actual time=0.0125..0.013 rows=5 loops=1)
        -> Filter: (locate('管理者',dtb_member.`name`) > 0)  (cost=0.75 rows=5) (actual time=0.00387..0.00762 rows=5 loops=1)
            -> Table scan on dtb_member  (cost=0.75 rows=5) (actual time=0.0035..0.00667 rows=5 loops=1)
