...
- **SQL File:** `admin_find_by_login.sql`
- **Cost:** 0.75
...
+- Table
   ...
   rows            5
   filtered        20.00
   condition       (`eccubedb_test`.`dtb_member`.`login_id` = 'admin1')
```
## Analysis Detail
...
### EXPLAIN JSON
{"select_id":1,"cost_info":{"query_cost":"0.75"},"table":{"table_name":"dtb_member","access_type":"ALL","rows_examined_per_scan":5,"rows_produced_per_join":1,"filtered":"20.00","cost_info":{"read_cost":"0.65","eval_cost":"0.10","prefix_cost":"0.75","data_read_per_join":"7K"},"used_columns":["id","work_id","authority_id","name","login_id","password","sort_no"],"attached_condition":"(`eccubedb_test`.`dtb_member`.`login_id` = 'admin1')"}}

### EXPLAIN ANALYZE
-> Limit: 1 row(s)  (cost=0.75 rows=1) (actual time=0.0085..0.00854 rows=1 loops=1)
    -> Filter: (dtb_member.login_id = 'admin1')  (cost=0.75 rows=1) (actual time=0.008..0.008 rows=1 loops=1)
        -> Table scan on dtb_member  (cost=0.75 rows=5) (actual time=0.00733..0.00733 rows=1 loops=1)
