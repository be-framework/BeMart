...
/**
 * Phase 3 — fidelity check for the Contact complete (goContactComplete)
 * HTML port.
 *
 * Same standard as {@see CartHtmlRenderTest}: BeMart's storefront
 * templates are PORTS of EC-CUBE 4.3's default-theme Twig.
 *
 * `Contact/complete.twig` is a DATA page (no form) — the data-page
 * recipe. It is the static inquiry-sent confirmation. The Complete
 * resource (src/Resource/Page/Contact/Complete.php) is a thin pure
 * renderer (a NEW resource — EC-CUBE's contact-complete screen had no
 * BEAR resource backing it). The residual is purely the
...
final class ContactCompleteHtmlRenderTest extends TestCase
{
    ...
    private const RESIDUAL_ALLOWLIST = [
        ...
        '});',
        '</script>',
        '<title>BeMart / お問い合わせ(完了)</title>',
        '<title>EC-CUBE / お問い合わせ(完了)</title>',
    ...
    {
        $beMart = $this->resource->get('page://self/contact/complete')->toString();
        ...
        $this->assertSame(
            [],
            $unexplained,
            "BeMart's contact-complete HTML diverged from EC-CUBE's beyond "
    ...
    private function renderEcCube(): string
    {
        ...
        return $twig->render('Contact/complete.twig', [
            ...
            ]), 'request' => new EcCubeStub(['_route' => 'contact_complete'])]),
            'subtitle' => 'お問い合わせ(完了)',
            'title' => 'お問い合わせ(完了)',
        ]);
    }
