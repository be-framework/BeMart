...
/**
 * Phase 3 — fidelity check for the Entry complete
 * (goCustomerRegistrationComplete) HTML port.
 *
 * Same standard as {@see CartHtmlRenderTest}: BeMart's storefront
 * templates are PORTS of EC-CUBE 4.3's default-theme Twig.
 *
 * `Entry/complete.twig` is a DATA page (no form) — the data-page recipe.
 * It is the static provisional-registration confirmation. The Complete
 * resource (src/Resource/Page/Entry/Complete.php) is a thin pure renderer
 * (a NEW resource — EC-CUBE's provisional-complete screen had no BEAR
 * resource backing it; BeMart's Pilot 4 implemented the
 * verification-OFF flow only). The residual is purely the
...
final class EntryCompleteHtmlRenderTest extends TestCase
{
    ...
    private const RESIDUAL_ALLOWLIST = [
        ...
        '});',
        '</script>',
        '<title>BeMart / 新規会員登録(仮登録完了)</title>',
        '<title>EC-CUBE / 新規会員登録(仮登録完了)</title>',
    ...
    public function testEntryCompletePageRendersAsHtmlDocument(): void
    {
    ...
    {
        $beMart = $this->resource->get('page://self/entry/complete')->toString();
        ...
        $this->assertSame(
            [],
            $unexplained,
            "BeMart's entry-complete HTML diverged from EC-CUBE's beyond "
    ...
    private function renderEcCube(): string
    {
        ...
        return $twig->render('Entry/complete.twig', [
            ...
            ]), 'request' => new EcCubeStub(['_route' => 'entry_complete'])]),
            'subtitle' => '新規会員登録(仮登録完了)',
            'title' => '新規会員登録(仮登録完了)',
        ]);
    }
