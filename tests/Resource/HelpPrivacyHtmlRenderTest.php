...
/**
 * Phase 3 — fidelity check for the Help/privacy (goHelpPrivacy) HTML
 * port.
 *
 * Same standard as {@see CartHtmlRenderTest}: BeMart's storefront
 * templates are PORTS of EC-CUBE 4.3's default-theme Twig.
 *
 * `Help/privacy.twig` ships a fixed Japanese prose body (no data
 * binding) — EC-CUBE's default privacy-policy copy. The Help/Privacy
 * resource (src/Resource/Page/Help/Privacy.php) is a pure renderer; the
 * prose is part of the template port, copied verbatim from EC-CUBE so
 * the render-diff is zero. The residual is the genuinely
...
final class HelpPrivacyHtmlRenderTest extends TestCase
{
    ...
    private const RESIDUAL_ALLOWLIST = [
        ...
        '});',
        '</script>',
        '<title>BeMart / プライバシーポリシー</title>',
        '<title>EC-CUBE / プライバシーポリシー</title>',
    ...
    public function testHelpPrivacyPageRendersAsHtmlDocument(): void
    {
    ...
    public function testHelpPrivacyPagePreservesEcCubeMarkupStructure(): void
    {
    ...
    /**
     * The honesty test: diff BeMart's rendered Help/privacy page against
    ...
    public function testHelpPrivacyHtmlMatchesEcCubeRenderingWithinResidualAllowlist(): void
    {
        ...
        $ecCube = $this->renderEcCubeHelpPrivacy();
        ...
        $this->assertSame(
            [],
            $unexplained,
            "BeMart's Help/privacy HTML diverged from EC-CUBE's beyond the "
    ...
    /**
     * Render EC-CUBE 4.3's real Help/privacy.twig + default_frame.twig
     * from the gitignored clone, with EC-CUBE's Twig API stubbed.
     */
    private function renderEcCubeHelpPrivacy(): string
    {
        ...
        return $twig->render('Help/privacy.twig', [
            ...
            ]), 'request' => new EcCubeStub(['_route' => 'help_privacy'])]),
            'subtitle' => 'プライバシーポリシー',
            'title' => 'プライバシーポリシー',
        ]);
    }
