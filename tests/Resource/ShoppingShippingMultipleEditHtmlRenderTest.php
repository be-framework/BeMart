...
/**
 * Phase 3 — fidelity check for the Shopping shipping-multiple-edit
 * (goShoppingShippingMultipleEdit) HTML port.
 *
 * Same standard as {@see CartHtmlRenderTest}: BeMart's storefront
 * templates are PORTS of EC-CUBE 4.3's default-theme Twig.
 *
 * `Shopping/shipping_multiple_edit.twig` is a FORM page — the "新規
 * お届け先を追加する" form reached from the multi-destination screen.
 * EC-CUBE renders its address inputs through the SAME `CustomerAddressType`
 * used by `Shopping/shipping_edit.twig`, so the BeMart port reuses the
 * same {@see ShoppingShippingEditForm} (an AbstractForm). The
 * ShippingMultipleEdit resource
 * (src/Resource/Page/Shopping/ShippingMultipleEdit.php) is a Wave-future
 * thin pure renderer that exposes the form as `body.form`. This test
 * renders EC-CUBE's `form_widget(form.name.name01)` calls through the
 * SAME form instance so the inputs diff to ZERO; `form_label` is stubbed
 * to the same `<label class="ec-label">` BeMart authors plainly. The
 * residual is the genuinely EC-CUBE-runtime-only `<head>` frame material
 * + the empty CSRF hidden value.
 */
final class ShoppingShippingMultipleEditHtmlRenderTest extends TestCase
{
    ...
    public function testShippingMultipleEditRendersRealFormInputs(): void
    {
        $html = $this->resource->get('page://self/shopping/shipping-multiple-edit')->toString();
    ...
    public function testShippingMultipleEditHtmlMatchesEcCubeRenderingWithinResidualAllowlist(): void
    {
        ...
        $this->assertSame(
            [],
            $unexplained,
            "BeMart's Shopping shipping-multiple-edit HTML diverged from "
    ...
    private function renderEcCube(): string
    {
        ...
        return $twig->render('Shopping/shipping_multiple_edit.twig', [
            'form' => new EcCubeStub([
            ...
            ]), 'request' => new EcCubeStub(['_route' => 'shopping_shipping_multiple_edit'])]),
            'subtitle' => 'お届け先の追加',
            'title' => 'お届け先の追加',
        ]);
    }
