...
final class HttpSqlAdminProductClassFormTest extends TestCase
{
    ...
    private int|null $seededProductId = null;
    ...
    protected function setUp(): void
    {
        ...
        $this->seedParentProductClass($this->pdo());
    }

    protected function tearDown(): void
    {
        $pdo = $this->pdo();
        $pdo->exec("DELETE FROM dtb_product_class WHERE product_code IN ('admin-active-001', 'admin-active-001-http')");
        if ($this->seededProductId !== null) {
            $pdo->exec(sprintf('DELETE FROM dtb_product WHERE id = %d', $this->seededProductId));
        }
    }

    /**
     * `dtb_product_class` FKs `product_id` to `dtb_product`; this real-browser
     * regression needs both rows to exist before the GET even renders, so it
     * seeds the parent product plus the `admin-active-001` class row it edits.
     * Self-healing: clears any leftover rows from an interrupted previous run
     * before inserting, so the test is safe to re-run without manual cleanup.
     */
    private function seedParentProductClass(PDO $pdo): void
    {
        $pdo->exec("DELETE FROM dtb_product_class WHERE product_code IN ('admin-active-001', 'admin-active-001-http')");
        $pdo->exec("DELETE FROM dtb_product WHERE name = 'HttpSqlAdminProductClassFormTest fixture'");

        $pdo->exec(
            "INSERT INTO dtb_product (product_status_id, name, create_date, update_date, discriminator_type) "
            . "VALUES (1, 'HttpSqlAdminProductClassFormTest fixture', NOW(), NOW(), 'product')",
        );
        $this->seededProductId = (int) $pdo->lastInsertId();

        $pdo->exec(sprintf(
            'INSERT INTO dtb_product_class ('
            . 'product_id, sale_type_id, product_code, price02, stock, stock_unlimited, visible, create_date, update_date, discriminator_type'
            . ") VALUES (%d, 1, 'admin-active-001', 1000, 50, 0, 1, NOW(), NOW(), 'productclass')",
            $this->seededProductId,
        ));
    }
    ...
    public function testRegisterProductClassFromRealBrowserWritesNewRow(): void
    {
        ...
        $this->assertStringContainsString('id="idea-product-class-form"', $edit['body']);
        // The 追加 submit must be present (the end-to-end affordance).
        $this->assertStringContainsString('追加', $edit['body']);
