...
/**
 * SQL-backed browser-form regression for admin page maintenance.
 */
final class HttpSqlAdminPageFormTest extends TestCase
{
    private const HOST = '127.0.0.1:18194';
    private const ADMIN_ID = '1';
    private const CSRF_TOKEN = 'admin-page-form-csrf-token';
    ...
    {
        $this->cookieJar = (string) tempnam(sys_get_temp_dir(), 'bemart-sql-admin-page-cookie-');
    }

    public function testAdminPageCanBeCreatedUpdatedAndDeletedThroughHtmlForms(): void
    {
        ...
        $this->assertStringContainsString('name="pageFileName"', $new['body']);
        $this->assertStringContainsString('id="main_edit_tpl_data"', $new['body']);
        $this->assertStringContainsString('disabled="disabled"', $new['body']);

        $suffix = bin2hex(random_bytes(4));
        $pageName = 'HTTPページ' . $suffix;
        $pageUrl = 'http-page-' . $suffix;
        $pageFileName = 'http_page_' . $suffix;
        $created = $this->form('POST', '/admin/page/page-list', [
            'pageName' => $pageName,
            'pageUrl' => $pageUrl,
        ...
        $this->assertStringStartsWith('/admin/page/page?pageId=', $location);
        $pageId = $this->pageIdFromLocation($location);
        ...
        $updatedName = 'HTTPページ更新' . $suffix;
        $updatedUrl = 'http-page-updated-' . $suffix;
        $updatedFileName = 'http_page_updated_' . $suffix;
        $updated = $this->form('POST', '/admin/page/page?pageId=' . $pageId . '&_method=put', [
            'pageName' => $updatedName,
            'pageUrl' => $updatedUrl,
            'pageFileName' => $updatedFileName,
            'csrfToken' => $this->csrfToken($detail['body']),
        ]);

        $this->assertSame(303, $updated['status'], $updated['body']);
        $this->assertSame('/admin/page/page?pageId=' . $pageId, $updated['headers']['Location'] ?? null);

        $updatedDetail = $this->request('GET', '/admin/page/page?pageId=' . $pageId);
        ...
        $this->assertStringContainsString('value="' . $updatedUrl . '"', $updatedDetail['body']);
        $this->assertStringContainsString('value="' . $updatedFileName . '"', $updatedDetail['body']);

        $listBeforeDelete = $this->request('GET', '/admin/page/page-list');
        $this->assertSame(200, $listBeforeDelete['status'], $listBeforeDelete['body']);
        $this->assertStringContainsString($updatedName, $listBeforeDelete['body']);
        $this->assertStringContainsString('/admin/page/page?pageId=' . $pageId . '&_method=delete', $listBeforeDelete['body']);

        $deleted = $this->form('POST', '/admin/page/page?pageId=' . $pageId . '&_method=delete', [
            'csrfToken' => self::CSRF_TOKEN,
        ]);

        $this->assertSame(303, $deleted['status'], $deleted['body']);
        $this->assertSame('/admin/page/page-list', $deleted['headers']['Location'] ?? null);

        $list = $this->request('GET', '/admin/page/page-list');
        $this->assertSame(200, $list['status'], $list['body']);
        $this->assertStringNotContainsString($updatedName, $list['body']);
    }
    ...
    private function pageIdFromLocation(string $location): string
    {
        $this->assertSame(1, preg_match('/[?&]pageId=([^&]+)/', $location, $match), $location);
