...
final class AuraRouterAdapterTest extends TestCase
{
    ...
    public function testMypageWithdrawConfirmModeRoutesToConfirmRenderer(): void
    {
        $match = $this->router->match(
            ['_GET' => [], '_POST' => ['mode' => 'confirm']],
            ['REQUEST_METHOD' => 'POST', 'REQUEST_URI' => '/mypage/withdraw'],
        );

        $this->assertSame('get', $match->method);
        $this->assertSame('page://self/mypage/withdraw-confirm', $match->path);
        $this->assertSame([], $match->query);
    }

    public function testAdminCustomizeCssAndJsPostRouteToConcreteResources(): void
    {
        $css = $this->router->match(
            ['_GET' => [], '_POST' => ['css' => 'body{}']],
            ['REQUEST_METHOD' => 'POST', 'REQUEST_URI' => '/admin_content_css'],
        );
        $js = $this->router->match(
            ['_GET' => [], '_POST' => ['js' => 'console.log(1);']],
            ['REQUEST_METHOD' => 'POST', 'REQUEST_URI' => '/admin_content_js'],
        );

        $this->assertSame('post', $css->method);
        $this->assertSame('page://self/admin/content/css', $css->path);
        $this->assertSame('post', $js->method);
        $this->assertSame('page://self/admin/content/js', $js->path);
    }
