...
- デモ会員: `login-test@example.com` / `local-dev-member-password`
- デモ管理者: <http://localhost:8080/admin/login> — `test-admin` / `local-dev-admin-password`（2FA はデモ用に `123456` 固定）
...
> デモ専用構成です。DB はパスワードなし root（ホストにポート公開せず compose 内ネットワーク限定）、管理 2FA はデモ用固定コード `123456` を受理（`BEMART_DEV_LOGIN=1`）。**本番では使わないでください。**
