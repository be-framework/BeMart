...
この repo では、最小の具体例として [`cart-quantity.json`](/Users/akihito/git/ec-cube-alps/.migrate/packets/cart-quantity.json) を置いている。さらに、その上位 packet として [`cart-add-cart-item-input.json`](/Users/akihito/git/ec-cube-alps/.migrate/packets/cart-add-cart-item-input.json) を置き、`AddCartItemInput` が `Quantity` semantic に依存する形にしている。
